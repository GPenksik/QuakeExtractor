...
public class QMovement : MonoBehaviour
{
	...
	private const float PLAYER_HEIGHT = 1.2f;
	...
	void Update () 
	{
		...
		if (controller.isGrounded)
		{
			...
			Vector3 verticalOffset = new(0.0f, -0.5f, 0.0f);

			Vector3 destination = transform.position + move_direction*frame_time;

			if (!Physics.Raycast(destination, vector_down, out hit_surface, 0.93f)) {
				Debug.Log("Nothing below feet at destination");
				Debug.DrawRay(destination, vector_down, Color.red, 2.0f, true);
			}
